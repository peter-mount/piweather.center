...
\paragraph{unit}defines the unit we want the results to be in.
If not defined then the unit of the output of the aggregate function is used.

\paragraph{aggregates\dots}define the aggregations to perform, usually one or more of the following:
\begin{table}[ht]
    \begin{tabular}{l l}
        \toprule
        Aggregator & Function \\
...
\paragraph{reset every}defines when the metric being calculated should be reset to some default value.
...
The result of this is that two new metrics are generated using \Metric{ws90.humidity} as the source.
