/*
 * Intersection, the minimum pixel value in two images
 */
#include <gd.h>
#include <stdlib.h>
#include <stdint.h>
...
gdImagePtr imagefilter_intersect(gdImagePtr img1, gdImagePtr img2) {
    ...
    for (y = 0; y < sy; y++)
        for (x = 0; x < sx; x++) {
            ...
            r = min(gdImageRed(img1, c1), gdImageRed(img2, c2));
            g = min(gdImageGreen(img1, c1), gdImageGreen(img2, c2));
            b = min(gdImageBlue(img1, c1), gdImageBlue(img2, c2));
