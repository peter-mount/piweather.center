...
type cronTabParser struct {
	stringTokenType lexer.TokenType
}
...
func CronTabParser(l lexer.Definition) func(pl *lexer.PeekingLexer) (CronTab, error) {
	...
	parser := &cronTabParser{
		stringTokenType: symbols["String"],
	}

	return parser.parse
}

func (c *cronTabParser) parse(pl *lexer.PeekingLexer) (CronTab, error) {
	token := pl.Peek()
	if token.EOF() {
		return nil, participle.Errorf(token.Pos, "EOF expected crontab definition")
	}

	if token.Type != c.stringTokenType {
