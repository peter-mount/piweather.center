...
func scriptInit(q *Query, err error) (*Query, error) {
	if err == nil {
		s := &parserState{usingNames: util.NewStringSet()}
		err = NewBuilder[*parserState]().
			Query(queryInit).
			QueryRange(queryRangeInit).
			UsingDefinition(s.usingDefinitionInit).
			Select(selectInit).
			Expression(s.expressionInit).
			ExpressionModifier(s.expressionModifierInit).
			Metric(metricInit).
			Time(timeInit).
...
func queryInit(_ Visitor[*parserState], s *Query) error {
	return assertLimit(s.Pos, s.Limit)
}

func queryRangeInit(v Visitor[*parserState], q *QueryRange) error {
...
func selectInit(_ Visitor[*parserState], s *Select) error {
	return assertLimit(s.Pos, s.Limit)
}

func tableSelect(v Visitor[*parserState], t *TableSelect) error {
	var err error
	if t.Unit != nil {
		err = v.Unit(t.Unit)
	}
...
func metricInit(_ Visitor[*parserState], b *Metric) error {
...
func (p *parserState) usingDefinitionInit(v Visitor[*parserState], u *UsingDefinition) error {
...
func (p *parserState) expressionInit(_ Visitor[*parserState], s *Expression) error {
...
func (p *parserState) expressionModifierInit(v Visitor[*parserState], s *ExpressionModifier) error {
...
func windRoseInit(_ Visitor[*parserState], s *WindRose) error {
